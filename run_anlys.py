...
if FLAGS.task == 'cov_rank':
	...
	if FLAGS.task == '1d_nonlinear':
		...
		x_idx_left = input('Variate Lower Bound: ')
		x_idx_right = input('Variate Upper Bound: ')
		...
		if x_idx_left == '':
			...
			x_idx_left = float(x_idx_left)
		if x_idx_right == '':
			x_idx_right = math.ceil((mean[idx] + 3 * std[idx]) * factor / 10) * 10
		else:
			x_idx_right = float(x_idx_right)
		x_idx = np.linspace(x_idx_left, x_idx_right, 51)
