...
if FLAGS.mode == 'train':
	feature_dim = num_poly_feature(291, order=FLAGS.order)
	config = Config(feature_dim=feature_dim, num_category=7, hidden_dim=[], dropout=0.9)
	model = Model(config)
	config_valid = Config(feature_dim=feature_dim, num_category=7, hidden_dim=[], dropout=1.0)
